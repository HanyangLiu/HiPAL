...
class tcnEncoder(Layer):
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        for k in range(self.n_layers):
            ...
            if self.if_batch_norm: x = self.batch_norms[k](x)
...
class nestedLowerEncoding(Layer):
    ...
    def build(self, input_shape):
        ...
        self.encoder = tcnEncoder(
            ...
            n_feat_in=self.embedding_matrix.shape[1] + self.time_size * 2,
            n_feat_out=self.embedding_matrix.shape[1] + self.time_size * 2,
...
class lowerModel:
    ...
    def build(self):
        self.lower_decoder = tcnDecoder(
            ...
            kernel_size=5,
            dilations=[3 for i in range(self.tcn_layers)],
            n_feat_in=self.embedding_matrix.shape[1] + 2 * self.time_size,
            n_feat_out=self.embedding_matrix.shape[1] + 2 * self.time_size,
            max_len=self.max_shift_len,
            dropout_rate=0.3,
            if_batch_norm=False
        )
...
class higherModel:
    ...
    def call(self):
        ...
        #     nb_filters=64,
        #     kernel_size=5,
        #     dilations=[2 ** i for i in range(self.tcn_layers)],
        #     n_feat_in=self.embedding_matrix.shape[1] + self.time_size * 2 + 32,
        #     n_feat_out=self.embedding_matrix.shape[1] + self.time_size * 2,
        #     max_len=self.max_shift_len,
        #     dropout_rate=self.dropout,
        #     if_layer_norm=False
        # )
        # embeded = layers.TimeDistributed(self.data_embedding, name='embedding')(input_1)
        # x_4 = tf.reduce_mean(embeded, axis=2, name='session_mean')
        # cond = layers.LSTM(32, return_sequences=True, name='lstm_cond')(x_4)
        # cond = tf.reshape(cond, shape=(-1, 30, 1, 32), name='session_expand')
        # cond = tf.tile(cond, [1, 1, 3008, 1], name='session_replicate')
        # combinded = layers.Concatenate(axis=3, name='cond_agg')([embeded, cond])
        # x_1 = layers.TimeDistributed(self.encoder, name='nested_base_model')(combinded)
        # ###################
