...
def train_inner_model(train_generator, test_generator, args):
    ...
    print(args)
    model.summary()
    tf.keras.utils.plot_model(model, show_shapes=True, expand_nested=True, to_file='result/nestedTCN.png')
...
def train_outer_model(train_generator, test_generator, args):
    ...
    print(args)
    model.summary()
    tf.keras.utils.plot_model(model, show_shapes=True, expand_nested=True, to_file='result/hierTCN.png')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--memo', type=str, default='CV_hitcn_wo_dilation')
    ...
    parser.add_argument('--bs_higher', type=int, default=2)
    ...
    parser.add_argument('--if_pretrain', type=boolean_string, default=True)
    parser.add_argument('--pretrain_model', type=str, default='fcn_testing')
    ...
    parser.add_argument('--low_encoder', type=str, default='fcn')
    parser.add_argument('--horizon', type=int, default=0)
    parser.add_argument('--trial_idx', type=int, default=1)
    parser.add_argument('--cv_idx', type=int, default=4)
    ...
    print('\n--------- Trial index: {} ---------\nFold index (0-4): {}'.format(args.trial_idx, args.cv_idx))
    ...
    # define model
    if args.low_encoder == 'tcn':
        from utils.util_hitcn import buildHierTCN, inner_ce_loss
    elif args.low_encoder == 'fcn':
        from utils.util_hifcn import buildHierTCN, inner_ce_loss
    args.built_model = buildHierTCN(args, embedding_matrix)
